<template>
  <div>
    <v-card color="indigo" dark elevation="0" outlined shaped>
      <v-card-title dark> Presets </v-card-title>
      <v-tabs
        v-model="tab"
        align-with-title
      ...
      <v-tabs-items v-model="tab">
        <v-tab-item class="pb-10 pt-8">
          <v-row class="mx-4">
            <v-col v-for="(elevation, index) in elevations" v-bind:key="index">
        ...
        <v-tab-item class="pb-10 pt-8">
          <v-row class="mx-4">
            <v-col
              v-for="(elevation, index) in tailwindElevations"
              v-bind:key="index"
            >
              <ShadowCard :elevation="elevation" />
            </v-col>
...
<script>
import { mdc, tailwind } from '../utils/presets'

export default {
  data() {
    return {
      tab: null,
      tabs: ['Material Design', 'Tailwind'],
    }
  },
  computed: {
    elevations() {
      return mdc().elevations
    },
    tailwindElevations() {
